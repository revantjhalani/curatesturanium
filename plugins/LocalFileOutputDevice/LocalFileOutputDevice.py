...
class LocalFileOutputDevice(OutputDevice):
    ...
    def requestWrite(self, nodes, file_name = None, limit_mimetypes = None, file_handler = None, **kwargs):
        ...
        try:
            ...
            message = Message(catalog.i18nc("@info:progress Don't translate the XML tags <filename>!", "Saving to <filename>{0}</filename>").format(save_name),
...
#            os.remove('/home/akshay/STLS/*')
    ...
    def _onWriteJobFinished(self, job):
        ...
        if job.getResult():
            ...
            message = Message(catalog.i18nc("@info:status Don't translate the XML tags <filename>!", "Saved to <filename>{0}</filename>").format((job.getFileName()).replace(".gcode",".s3d")), title = catalog.i18nc("@info:title", "File Saved"))
